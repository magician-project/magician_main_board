		...
		VL53L5CX_Configuration		*p_dev)
{
	...
	uint8_t b02_status = VL53L5CX_STATUS_OK;
	uint8_t b05_status = VL53L5CX_STATUS_OK;
	...
	b02_status = VL53L5CX_STATUS_ERROR;

	for (uint8_t retry = 0; retry < 5; retry++)
	{
		uint8_t b02_try_status = VL53L5CX_STATUS_OK;
		uint8_t rb_after_first_40 = 0xFF;
		uint8_t rb_after_43 = 0xFF;
		uint8_t rb_after_final_40 = 0xFF;

		b02_try_status |= WrByte(&(p_dev->platform), 0x7fff, 0x00);

		b02_try_status |= WrByte(&(p_dev->platform), 0x000F, 0x40);
		b02_try_status |= WaitMs(&(p_dev->platform), 1);
		b02_try_status |= RdByte(&(p_dev->platform), 0x000F,
			&rb_after_first_40);

		b02_try_status |= WrByte(&(p_dev->platform), 0x000A, 0x03);
		b02_try_status |= WrByte(&(p_dev->platform), 0x000B, 0x01);
		b02_try_status |= WrByte(&(p_dev->platform), 0x000C, 0x00);
		b02_try_status |= WrByte(&(p_dev->platform), 0x000D, 0x00);

		b02_try_status |= WrByte(&(p_dev->platform), 0x000E, 0x00);

		b02_try_status |= WrByte(&(p_dev->platform), 0x000F, 0x43);
		b02_try_status |= WaitMs(&(p_dev->platform), 1);
		b02_try_status |= RdByte(&(p_dev->platform), 0x000F,
			&rb_after_43);
		...
		if ((b02_try_status == VL53L5CX_STATUS_OK) &&
			(rb_after_first_40 == 0x40) &&
			(rb_after_43 == 0x43) &&
			(rb_after_final_40 == 0x40))
		{
			b02_status = VL53L5CX_STATUS_OK;
			break;
		}

		(void)WaitMs(&(p_dev->platform), 100);
	}
	...
	/* Enable FW access.
	 * On some boot cycles, the first poll on register 0x0021 may remain
	 * at 0x00. Repeating the complete enable sequence allows the internal
	 * state machine to settle correctly.
	 */

	b05_status = VL53L5CX_STATUS_ERROR;

	for (uint8_t retry = 0; retry < 3; retry++)
	{
		b05_status = VL53L5CX_STATUS_OK;

		b05_status |= WrByte(&(p_dev->platform), 0x000E, 0x01);
		b05_status |= WrByte(&(p_dev->platform), 0x7fff, 0x02);

		b05_status |= WrByte(&(p_dev->platform), 0x03, 0x0D);
		b05_status |= WrByte(&(p_dev->platform), 0x7fff, 0x01);

		b05_status |= WaitMs(&(p_dev->platform), 5);

		b05_status |= _vl53l5cx_poll_for_answer(
			p_dev, 1, 0, 0x21, 0x10, 0x10);

		if (b05_status == VL53L5CX_STATUS_OK)
		{
			break;
		}

		(void)WrByte(&(p_dev->platform), 0x7fff, 0x00);
		(void)WaitMs(&(p_dev->platform), 20);
	}
